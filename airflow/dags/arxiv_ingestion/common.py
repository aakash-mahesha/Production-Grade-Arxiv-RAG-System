...
def get_cached_services() -> Tuple[Any, Any, Any, Any, Any]:
    ...
    Imports are deferred to avoid slow DAG parsing (Docling loads ML models).

    :returns: Tuple of (arxiv_client, pdf_parser, database, metadata_fetcher, opensearch_client)
    """
    # Lazy imports - only load heavy modules when task actually runs
    # This prevents DAG import timeout (Docling loads ML models which takes >30s)
